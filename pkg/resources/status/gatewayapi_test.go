...
var (
	...
	expectDNSReadyOnStatus = func(ready bool, generation int64) []metav1.Condition {
...
func TestComputeGatewayAPIListenerDNSStatus(t *testing.T) {
	tests := []struct {
		...
		{
			name:               "a null listenerToHostname map should not mutate the listeners status nor Gateway status",
			generation:         1,
		...
		},
		{
			...
			expectedGatewayConditions: expectDNSReadyOnStatus(false, 2),
		},
		{
			...
			expectedGatewayConditions: expectDNSReadyOnStatus(false, 3),
		},
		{
			...
			expectedGatewayConditions: expectDNSReadyOnStatus(false, 4),
		},
		{
			...
			expectedGatewayConditions: expectDNSReadyOnStatus(false, 5),
		},
		...
		{
			name:                "a non-matching listener hostname should not mutate the listener status or gateway status",
			generation:          1,
			dnsConfig:           defaultDNSConfig,
			listenerToHostname:  listenerToHostname,
			hostnameToDNSRecord: hostnameToDNSRecord,
		...
		{
			name:                "no DNSReady condition on listener level must remove the DNSReady from the gateway level",
			generation:          1,
			dnsConfig:           defaultDNSConfig,
			listenerToHostname:  listenerToHostname,
			hostnameToDNSRecord: hostnameToDNSRecord,
		...
		},
		{
			...
			expectedGatewayConditions: expectDNSReadyOnStatus(false, 6),
		},
		{
			...
			expectedGatewayConditions: expectDNSReadyOnStatus(false, 7),
		},
		...
		// being granular to listeners
		{
			...
			expectedGatewayConditions: expectDNSReadyOnStatus(false, 8),
		},
		{
			...
			expectedGatewayConditions: expectDNSReadyOnStatus(true, 9),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.name == "no DNSReady condition on listener level must remove the DNSReady from the gateway level" {
				// do nothing
			}
			status.ComputeGatewayAPIDNSStatus(tt.dnsConfig, tt.generation, tt.gwstatus, tt.listenerToHostname, tt.hostnameToDNSRecord)
			...
			if !cmp.Equal(tt.gwstatus.Conditions, tt.expectedGatewayConditions, conditionsCmpOpts...) {
				t.Errorf("diff: %s", cmp.Diff(tt.expectedGatewayConditions, tt.gwstatus.Conditions, conditionsCmpOpts...))
			}
		})
	}
}
