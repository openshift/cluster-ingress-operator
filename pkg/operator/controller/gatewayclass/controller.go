...
func NewUnmanaged(mgr manager.Manager, config Config) (controller.Controller, error) {
	...
	if err := c.Watch(source.Kind(operatorCache, &gatewayapiv1.GatewayClass{}), &handler.EnqueueRequestForObject{}, isOurGatewayClass, predicate.Not(isIstioGatewayClass)); err != nil {
		return nil, err
	}
	return c, nil
}
