...
func getNamedInitContainer(t *testing.T, deployment *appsv1.Deployment, name string) *corev1.Container {
	t.Helper()
	...
	require.FailNow(t, fmt.Sprintf("%s container not found", name))
	return nil
}

func checkContainerSecurityContexts(t *testing.T, deployment *appsv1.Deployment) {
	t.Helper()

	// Check the router container has readOnlyRootFilesystem.
	routerContainer := &deployment.Spec.Template.Spec.Containers[0]
	require.Equal(t, routerContainerName, routerContainer.Name, "expected first container to be %q", routerContainerName)
	require.NotNil(t, routerContainer.SecurityContext, "containers[%s].securityContext", routerContainerName)
	if readOnlyFS := routerContainer.SecurityContext.ReadOnlyRootFilesystem; readOnlyFS == nil || !*readOnlyFS {
		t.Errorf("%s container does not have securityContext.readOnlyRootFilesystem: true", routerContainerName)
	}

	// Check the init-router container has readOnlyRootFilesystem.
	initContainer := getNamedInitContainer(t, deployment, routerInitContainerName)
	require.NotNil(t, initContainer.SecurityContext, "initContainers[%s].securityContext", routerInitContainerName)
	if readOnlyFS := initContainer.SecurityContext.ReadOnlyRootFilesystem; readOnlyFS == nil || !*readOnlyFS {
		t.Errorf("%s container does not have securityContext.readOnlyRootFilesystem: true", routerInitContainerName)
	}

	// Check the haproxy container has allowPrivilegeEscalation and readOnlyRootFilesystem.
	haproxyContainer := getNamedInitContainer(t, deployment, routerHAProxyContainerName)
	require.NotNil(t, haproxyContainer.SecurityContext, "initContainers[%s].securityContext", routerHAProxyContainerName)
	if allowPrivEsc := haproxyContainer.SecurityContext.AllowPrivilegeEscalation; allowPrivEsc == nil || !*allowPrivEsc {
		t.Errorf("%s container does not have securityContext.allowPrivilegeEscalation: true", routerHAProxyContainerName)
	}
	if readOnlyFS := haproxyContainer.SecurityContext.ReadOnlyRootFilesystem; readOnlyFS == nil || !*readOnlyFS {
...
func checkHAProxyContainerSocketRef(t *testing.T, deployment *appsv1.Deployment) {
	t.Helper()
	container := getNamedInitContainer(t, deployment, routerHAProxyContainerName)
...
func TestTuningOptions(t *testing.T) {
	...
	deployment, err := desiredRouterDeployment(ic, &Config{IngressControllerImage: ingressControllerImage}, ingressConfig, infraConfig, apiConfig, networkConfig, nil, false, false, nil, clusterProxyConfig, 2)
	if err != nil {
...
func TestTuningOptionForAWSNLB(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			deployment, err := desiredRouterDeployment(ic, &Config{IngressControllerImage: ingressControllerImage}, ingressConfig, infraConfig, apiConfig, networkConfig, test.lbService, false, false, nil, clusterProxyConfig, 2)
			if err != nil {
				t.Fatalf("invalid router Deployment: %v", err)
			}
...
func Test_desiredRouterDeployment(t *testing.T) {
	...
	deployment, err := desiredRouterDeployment(ic, &Config{IngressControllerImage: ingressControllerImage}, ingressConfig, infraConfig, apiConfig, networkConfig, nil, proxyNeeded, false, nil, clusterProxyConfig, 2)
	if err != nil {
		t.Fatalf("invalid router Deployment: %v", err)
	}

	checkContainerSecurityContexts(t, deployment)
...
func TestDesiredRouterDeploymentSpecHAProxyVersion(t *testing.T) {
	...
	for name, test := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			deployment, err := desiredRouterDeployment(ic, deploymentConfig, ingressConfig, infraConfig, apiConfig, networkConfig, nil, proxyNeeded, false, nil, clusterProxyConfig, 2)
...
func TestDesiredRouterDeploymentFIPS(t *testing.T) {
	...
	isFIPSEnabled = false
	deploymentNonFIPS, err := desiredRouterDeployment(ic, &Config{IngressControllerImage: ingressControllerImage}, ingressConfig, infraConfig, apiConfig, networkConfig, nil, proxyNeeded, false, nil, clusterProxyConfig, 2)
	if err != nil {
...
func TestDesiredRouterDeploymentClientTLS(t *testing.T) {
	...
	deployment, err := desiredRouterDeployment(ic, &Config{IngressControllerImage: ingressControllerImage}, ingressConfig, infraConfig, apiConfig, networkConfig, nil, false, true, clientCAConfigmap, clusterProxyConfig, 2)
	if err != nil {
		t.Fatalf("invalid router deployment: %v", err)
	}
...
func TestDesiredRouterDeploymentDynamicConfigManager(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			deployment, err := desiredRouterDeployment(ic, &Config{IngressControllerImage: ingressControllerImage, IngressControllerDCMEnabled: tc.dcmEnabled}, &configv1.Ingress{}, &configv1.Infrastructure{}, &configv1.APIServer{}, &configv1.Network{}, nil, false, false, nil, &configv1.Proxy{}, 2)
			if err != nil {
				t.Error(err)
			}
...
func TestDesiredRouterDeploymentMutualTLSHeaderFilter(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			deployment, err := desiredRouterDeployment(ic, &Config{IngressControllerImage: ingressControllerImage}, &configv1.Ingress{}, &configv1.Infrastructure{}, &configv1.APIServer{}, &configv1.Network{}, nil, false, false, nil, &configv1.Proxy{}, 2)
...
func TestDesiredRouterDeploymentDefaultPlacement(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			deployment, err := desiredRouterDeployment(ic, &Config{IngressControllerImage: ingressControllerImage}, tc.ingressConfig, tc.infraConfig, apiConfig, networkConfig, nil, dummyProxyNeeded, false, nil, &configv1.Proxy{}, 2)
			if err != nil {
				t.Error(err)
			}
