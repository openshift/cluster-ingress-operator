...
func SetNLBHairpinRiskMetric(ic *operatorv1.IngressController) {
	...
	if status != nil &&
		...
		status.LoadBalancer.ProviderParameters.AWS.NetworkLoadBalancerParameters.Protocol == operatorv1.NLBProtocolTCP &&
		getAWSNLBProtocol(ic.Spec.EndpointPublishingStrategy) == "" {
		labelVal = 1
	}
