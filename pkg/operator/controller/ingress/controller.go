...
func New(mgr manager.Manager, config Config) (controller.Controller, error) {
	...
	if config.ControlPlaneTopology == configv1.ExternalTopologyMode {
		// Watch for changes that can make a router pod schedulable. Node status
		// updates are filtered below so regular node heartbeats do not enqueue
...
type Config struct {
	...
	// ControlPlaneTopology is used to register the Node watch only for
	// external-control-plane clusters, where guest nodes may join after the
	// ingress operator starts.
...
func (r *reconciler) ensureIngressController(ci *operatorv1.IngressController, dnsConfig *configv1.DNS, infraConfig *configv1.Infrastructure, platformStatus *configv1.PlatformStatus, ingressConfig *configv1.Ingress, apiConfig *configv1.APIServer, networkConfig *configv1.Network, clusterProxyConfig *configv1.Proxy) error {
	...
	// The node list is used only for external-control-plane clusters to
	// distinguish the initial HCP state, where the ingress operator can be
	// ready before guest worker nodes exist, from a later router outage. A list
	// failure must not be treated as an empty list.
	...
	if infraConfig.Status.ControlPlaneTopology == configv1.ExternalTopologyMode {
		...
		if err := r.cache.List(context.TODO(), nodes); err != nil {
			errs = append(errs, fmt.Errorf("failed to list nodes: %w", err))
			nodes = nil
		}
	}
	...
	if haveLB && ci.Name == manifests.DefaultIngressControllerName {
		// For platforms with ClusterHosted DNS, only update Infrastructure API after machine-config is ready.
		// Background: MCO uses the infrastructure CR to generate its master ignition. Adding the ingress
		// LB IPs before MCO has done that would cause the ignition file generated by the MCO to be
		// different from the one generated by the Installer (without the ingress LB IPs). This race
		// prevents masters (usually master 1 or 2) from becoming ready.
		// This fix would cause a reboot of the master nodes when the Infra CR is eventually mutated but
		// we are OK with that.
		if needsClusterHostedDNS(infraConfig) {
			if !isMCOReady(context.TODO(), r.client) {
				log.Info("Skipping Infrastructure status update: machine-config not yet ready")
			} else {
...
func needsClusterHostedDNS(infraConfig *configv1.Infrastructure) bool {
	platformStatus := infraConfig.Status.PlatformStatus
	if platformStatus == nil {
		return false
	}
	...
	switch platformStatus.Type {
	case configv1.AWSPlatformType:
		return platformStatus.AWS != nil &&
			platformStatus.AWS.CloudLoadBalancerConfig != nil &&
			platformStatus.AWS.CloudLoadBalancerConfig.DNSType == configv1.ClusterHostedDNSType
	case configv1.AzurePlatformType:
		return platformStatus.Azure != nil &&
			platformStatus.Azure.CloudLoadBalancerConfig != nil &&
			platformStatus.Azure.CloudLoadBalancerConfig.DNSType == configv1.ClusterHostedDNSType
	case configv1.GCPPlatformType:
		return platformStatus.GCP != nil &&
			platformStatus.GCP.CloudLoadBalancerConfig != nil &&
			platformStatus.GCP.CloudLoadBalancerConfig.DNSType == configv1.ClusterHostedDNSType
	default:
		return false
	}
...
func isMCOReady(ctx context.Context, client client.Client) bool {
	...
	if err := client.Get(ctx, types.NamespacedName{Name: "machine-config"}, mco); err != nil {
		// If machine-config is not available, assume it's not ready
		if kerrors.IsNotFound(err) {
			log.V(2).Info("failed to get machine-config ClusterOperator, assuming not ready", "error", err)
		} else {
			log.Error(err, "failed to get machine-config ClusterOperator")
		}
		return false
	}
