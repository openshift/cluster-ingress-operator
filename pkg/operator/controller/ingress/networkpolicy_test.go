...
func Test_desiredRouterNetworkPolicy(t *testing.T) {
	...
	np := desiredRouterNetworkPolicy(ic, ownerRef)
	...
	want := map[string]struct{}{"http": {}, "https": {}, "metrics": {}}
	got := map[string]struct{}{}
	for _, r := range np.Spec.Ingress {
		for _, p := range r.Ports {
			if p.Protocol != nil && *p.Protocol == corev1.ProtocolTCP && p.Port != nil && p.Port.StrVal != "" {
				got[p.Port.StrVal] = struct{}{}
			}
		}
	}
	for port := range want {
		if _, ok := got[port]; !ok {
			t.Fatalf("expected ingress port %s to be present", port)
		}
	}
}
