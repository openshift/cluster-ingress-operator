...
func TestSetDefaultPublishingStrategyHandlesUpdates(t *testing.T) {
	var (
		...
		nlbTcpWithNullParameters = func() *operatorv1.EndpointPublishingStrategy {
			eps := eps(lbs(operatorv1.ExternalLoadBalancer, &managedDNS))
			eps.LoadBalancer.ProviderParameters = &operatorv1.ProviderLoadBalancerParameters{
				Type: operatorv1.AWSLoadBalancerProvider,
				AWS: &operatorv1.AWSLoadBalancerParameters{
	...
	testCases := []struct {
		...
		},
		{
			...
			ic:                      makeIC(spec(nlbProxyWithNullParameters()), status(nlbWithNullParameters())),
			expectedResult:          true,
			expectedIC:              makeIC(spec(nlbProxyWithNullParameters()), status(nlbProxyWithNullParameters())),
			domainMatchesBaseDomain: true,
		},
		{
			...
			expectedResult:          true,
			expectedIC:              makeIC(spec(nlbWithNullParameters()), status(nlbProxyWithNullParameters())),
			domainMatchesBaseDomain: true,
		},
		{
