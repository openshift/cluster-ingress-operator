...
func loadBalancerServiceChanged(current, expected *corev1.Service) (bool, *corev1.Service) {
	...
	// Reconcile the target group attributes annotation only when the desired service specifies it.
	// This is one-directional: it adds or updates the annotation but never removes it. The annotation
	// is not in managedLBServiceAnnotations because that set deletes annotations absent from the
	// desired service, which would stomp user-set values.
...
func awsSecurityGroupsEqual(securityGroups1, securityGroups2 []operatorv1.SecurityGroupID) bool {
	...
	sgCounts := make(map[operatorv1.SecurityGroupID]int, len(securityGroups1))
