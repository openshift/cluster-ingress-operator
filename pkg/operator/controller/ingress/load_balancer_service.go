...
func createEffectuationMessage(fieldName, haveValue, wantValue, revertPatchSpec string, ic *operatorv1.IngressController, service *corev1.Service) error {
	changedMsg := fmt.Sprintf("The IngressController %s was changed from %q to %q.",
	...
	if revertPatchSpec == "" {
		return fmt.Errorf("%s", changedMsg)
	}
	...
	return fmt.Errorf("%s", effectuationMsg)
}
...
func loadBalancerServiceIsProgressing(ic *operatorv1.IngressController, service *corev1.Service, platform *configv1.PlatformStatus) error {
	...
	if platform.Type == configv1.AWSPlatformType {
		...
		if wantLBType != "" && wantLBType != haveLBType {
			...
			err := createEffectuationMessage("load balancer type", string(haveLBType), string(wantLBType), patchSpec, ic, service)
			errs = append(errs, err)
		}
		...
		if haveLBType == operatorv1.AWSNetworkLoadBalancer {
			...
			if !awsEIPAllocationsEqual(wantEIPAllocations, haveEIPAllocations) {
				...
				patchSpec := fmt.Sprintf(`{"spec":{"endpointPublishingStrategy":{"type":"LoadBalancerService","loadBalancer":{"providerParameters":{"type":"AWS","aws":{"type":"%s","networkLoadBalancer":{"eipAllocations":%s}}}}}}}`, haveLBType, haveEIPAllocationsPatchJson)
