...
func Test_desiredLoadBalancerService(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			haveSvc, svc, err := desiredLoadBalancerService(&ingressServiceConditionInput{ic: ic, deploymentRef: deploymentRef, platform: infraConfig.Status.PlatformStatus, subnetsAWSEnabled: tc.subnetsAWSFeatureEnabled, eipAllocationsAWSEnabled: tc.eipAllocationsAWSFeatureEnabled, nlbSecurityGroupAWSEnabled: tc.securityGroupsAWSFeatureEnabled})
			switch {
			case err != nil:
				t.Error(err)
...
func TestDesiredLoadBalancerServiceAWSIdleTimeout(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			haveSvc, svc, err := desiredLoadBalancerService(&ingressServiceConditionInput{
				ic:                         ic,
...
func TestUpdateLoadBalancerServiceSourceRanges(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			wantSvc, desired, err := desiredLoadBalancerService(&ingressServiceConditionInput{
				ic:                         ic,
				deploymentRef:              deploymentRef,
				platform:                   infraConfig.Status.PlatformStatus,
				subnetsAWSEnabled:          true,
