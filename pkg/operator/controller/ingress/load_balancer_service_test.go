...
// Test_isAWSNLB verifies that isAWSNLB correctly identifies NLB status.
func Test_isAWSNLB(t *testing.T) {
	testCases := []struct {
		name     string
		lbStatus *operatorv1.LoadBalancerStrategy
		expect   bool
	}{
		{
			name:     "nil status",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.expect, isAWSNLB(tc.lbStatus))
		})
	}
}
