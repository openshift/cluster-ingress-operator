	...
	isDefaultIngressCert := predicate.NewPredicateFuncs(func(o client.Object) bool {
		if o.GetNamespace() == config.OperandNamespace {
			...
			if err := reconciler.client.Get(context.Background(), defaultICName, defaultIC); err != nil {
				log.Error(err, "failed to get default ingress controller")
				return false
			}
...
func (r *reconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	_, _, err := r.ensureCanaryCertificate(ctx)
	if err != nil {
...
// createCanaryCertificate creates the canary certificate, and returns an error
// if it cannot.
