...
func NewUnmanaged(mgr manager.Manager, config Config) (controller.Controller, error) {
	...
	if err := c.Watch(source.Kind(operatorCache, &gatewayapiv1.Gateway{}), handler.EnqueueRequestsFromMapFunc(gatewayToService), isInOperandNamespace, gatewayListenersChanged); err != nil {
		return nil, err
	}
