...
func (r *reconciler) getOperatorState(ctx context.Context, ingressNamespace, canaryNamespace string, co *configv1.ClusterOperator) (operatorState, error) {
	...
	if err := r.client.Get(ctx, types.NamespacedName{Name: ingressNamespace}, ingressConfig); err != nil {
		if !errors.IsNotFound(err) {
	...
	canaryConfig := &configv1.Ingress{}
	if err := r.client.Get(ctx, types.NamespacedName{Name: canaryNamespace}, canaryConfig); err != nil {
		if !errors.IsNotFound(err) {
			return state, fmt.Errorf("failed to get canary ingress: %v", err)
		}
	} else {
		state.IngressComponentRoutesStatus = canaryConfig.Status.ComponentRoutes
	}
...
// helper to append an ObjectReference only if an equivalent entry is not already present
