...
func Test_Reconcile(t *testing.T) {
	...
	tests := []struct {
		...
		// existingStatusSubresource contains objects whose status
		// is updated by Reconcile function.
		// This field is similar to `existingObjects` but is specifically used
		// for objects where status updates are peformed using `Status().X()` calls.
		existingStatusSubresource []client.Object
		expectCreate              []client.Object
		expectUpdate              []client.Object
		expectDelete              []client.Object
		// expectStatusUpdate contains objects whose status
		// is expected to be updated by the test.
