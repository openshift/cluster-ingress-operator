...
func getAuthorizerForResource(config Config) (autorest.Authorizer, error) {
	...
	cred, err := azidentity.NewClientSecretCredential(config.TenantID, config.ClientID, config.ClientSecret, &azidentity.ClientSecretCredentialOptions{ClientOptions: options})
	if err != nil {
		return nil, err
	}
	...
	authorizer := azidext.NewTokenCredentialAdapter(cred, []string{endpointToScope(config.Environment.ResourceManagerEndpoint)})
