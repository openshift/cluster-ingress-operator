...
func (p *Provider) Replace(record *iov1.DNSRecord, zone configv1.DNSZone) error {
	...
	if err := listCall.Pages(ctx, func(page *gdnsv1.ResourceRecordSetsListResponse) error {
		for _, rrs := range page.Rrsets {
			log.Info("found old DNS resource record set", "type", rrs.Type)
			oldRecords = append(oldRecords, rrs)
		}
		return nil
	}); err != nil {
		return fmt.Errorf("failed to list resource record sets for %s in zone %s: %w", record.Spec.DNSName, zoneID, err)
	}
	...
	if _, err := call.Do(); err != nil {
		...
		return fmt.Errorf("failed to replace DNS record %s in zone %s: %w", record.Spec.DNSName, zoneID, err)
	}
	return nil
}
