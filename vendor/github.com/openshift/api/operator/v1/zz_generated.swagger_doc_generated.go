...
var map_IngressControllerSpec = map[string]string{
	...
	"httpErrorCodePage":          "This used to sync the configmap(Eg- Admin first creates a configmap called http503page in openshift-config) from openshift-config to openshift-ingress The ingress operator configures the router deployment with a volume and volume mount that mounts the custom error pages from the httpErrorCodePage: http503page configmap on top of the existing error pages in the router pods. So we need a new IngressController API field in openshift/api, errorfile stanzas and error pages in openshift/router, and a new controller to copy the configmap and changes to desiredRouterDeployment to mount the configmap into the deployment in openshift/cluster-ingress-operator.",
