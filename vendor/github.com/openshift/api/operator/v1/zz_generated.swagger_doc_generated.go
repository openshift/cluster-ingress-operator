...
var map_OpenStackLoadBalancerParameters = map[string]string{
	...
	"loadBalancerIP": "loadBalancerIP specifies the floating IP address that the load balancer will use. A user can use that so they can specific which address to use for a given load balancer instead of getting a random one from the cloud-provider-openstack. This must be a valid IPv4 or IPv6 address.",
}
