...
var map_AWSNetworkLoadBalancerParameters = map[string]string{
	...
	"clientIPPreservationMode": "clientIPPreservationMode specifies how client IP addresses are preserved by the load balancer.\n\nValid values are \"Native\" and \"ProxyProtocol\".\n\nWhen set to \"Native\", the NLB uses AWS's native client IP preservation, which may cause hairpin connection failures for internal load balancers when connections are made from pods to router pods on the same node.\n\nWhen set to \"ProxyProtocol\", the NLB uses PROXY protocol v2 to preserve client IP addresses. This avoids hairpin connection failures.\n\nWhen omitted, this means the user has no opinion and the value is left to the platform to choose a good default, which is subject to change over time. The current default is \"ProxyProtocol\".\n\nNote that changing this field may cause brief connection failures during the transition as the NLB attribute change and router rollout occur independently.",
}
