...
// Test_IdleConnectionTerminationPolicy verifies both connection reuse
// and correct service responses when switching between backend
// services under different IdleConnectionTerminationPolicy settings.
//
// This test:
//
//  1. Deploys two backend services (`web-service-1` and `web-service-2`).
//
//  2. Switches a Route between the backends and makes a GET request.
//
//  3. For Immediate policy:
//
//     Step 1: Verify initial request is served by web-service-1.
//
//     Step 2: Switch route to web-service-2. Verify request is
//     immediately served by web-service-2.
//
//     Step 3: Verify subsequent request is also served by
//     web-service-2.
//
//     Verifies steps 2 and 3 use the same connection (same local
//     address), showing the new connection is maintained after the
//     backend switch.
//
//  4. For Deferred policy:
//
//     Step 1: Verify initial request is served by web-service-1.
//
//     Step 2: Switch route to web-service-2. Verify request is still
//     served by web-service-1 due to connection reuse.
//
//     Step 3: Verify subsequent request is served by web-service-2.
//
//     Verifies steps 1 and 2 use the same connection (same local
//     address), showing the original connection is maintained during
//     the backend switch.
func Test_IdleConnectionTerminationPolicy(t *testing.T) {
	...
	type testAction struct {
		description      string
		fetchResponse    func(httpClient *http.Client, elbAddress string, testConfig *testConfig) (string, string, error)
	...
	immediateActions := []testAction{
		{
			...
			fetchResponse: func(httpClient *http.Client, elbAddr string, cfg *testConfig) (string, string, error) {
		...
		},
		{
			...
			fetchResponse: func(httpClient *http.Client, elbAddress string, cfg *testConfig) (string, string, error) {
				if err := idleConnectionSwitchRouteService(t, cfg.routeName, cfg.ingressController.Name, webService2); err != nil {
					return "", "", err
				}
				return idleConnectionFetchResponse(t, httpClient, elbAddress, cfg.routeHost)
			},
	...
	for _, tc := range []struct {
		...
		t.Run(string(tc.policy), func(t *testing.T) {
			...
			testName := names.SimpleNameGenerator.GenerateName(strings.ToLower(fmt.Sprintf("idle-connection-close-%s-", tc.policy)))
			...
			ic, err := createIngressController(t, testNs, tc.policy)
			if err != nil {
			...
			case operatorv1.IngressControllerConnectionTerminationPolicyImmediate:
				if localAddresses[1] != localAddresses[2] {
					t.Fatalf("%s policy: expected steps 2 and 3 to use same connection, but got addresses %s and %s", tc.policy, localAddresses[1], localAddresses[2])
				}
			case operatorv1.IngressControllerConnectionTerminationPolicyDeferred:
				if localAddresses[0] != localAddresses[1] {
					t.Fatalf("%s policy: expected steps 1 and 2 to use same connection, but got addresses %s and %s", tc.policy, localAddresses[0], localAddresses[1])
				}
			}
		})
	}
