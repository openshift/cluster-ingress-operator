...
// - commitInterval
// - blueprintRouteNamespace
// - blueprintRouteLabels
// - blueprintRoutePoolSize
// - blueprintMaxDynamicServers
...
func TestUnsupportedConfigOverride(t *testing.T) {
	...
	var tests = []struct {
		name, unsupportedConfigOverrideName, unsupportedConfigOverrideValue, env string
	}{
		{"dynamic-config-manager", "dynamicConfigManager", "true", "ROUTER_HAPROXY_CONFIG_MANAGER"},
		{"commit-interval", "commitInterval", "5s", "COMMIT_INTERVAL"},
		{"blueprint-route-namespace", "blueprintRouteNamespace", "foo", "ROUTER_BLUEPRINT_ROUTE_NAMESPACE"},
		{"blueprint-route-labels", "blueprintRouteLabels", "foo=bar", "ROUTER_BLUEPRINT_ROUTE_LABELS"},
		{"blueprint-root-pool-size", "blueprintRoutePoolSize", "5", "ROUTER_BLUEPRINT_ROUTE_POOL_SIZE"},
		{"max-dynamic-servers", "maxDynamicServers", "5", "ROUTER_MAX_DYNAMIC_SERVERS"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ic.Spec.UnsupportedConfigOverrides = runtime.RawExtension{
				Raw: []byte(fmt.Sprintf(`{"%s":"%s"}`, tt.unsupportedConfigOverrideName, tt.unsupportedConfigOverrideValue)),
			}
			if err := kclient.Update(context.TODO(), ic); err != nil {
				t.Fatalf("failed to update ingresscontroller: %v", err)
			}
			if err := waitForDeploymentEnvVar(t, kclient, deployment, 1*time.Minute, tt.env, tt.unsupportedConfigOverrideValue); err != nil {
