...
func TestSyslogLogging(t *testing.T) {
	...
	image, err := getIngressControllerDeploymentImage(t, kclient, defaultName, 1*time.Minute)
	if err != nil {
		t.Fatal("failed to determine default ingresscontroller deployment's image: ", err)
	}
...
func TestServerTimeout(t *testing.T) {
	...
	if err := kclient.Create(context.TODO(), pod); err != nil {
		t.Fatalf("failed to create pod %s/%s: %v", pod.Namespace, pod.Name, err)
	}
	defer func() {
		if err := kclient.Delete(context.TODO(), pod); err != nil {
	...
	if err := wait.PollUntilContextTimeout(context.TODO(), 5*time.Second, 1*time.Minute, false, func(_ context.Context) (bool, error) {
		start := time.Now()
		response, err := client.Do(request)
		if err != nil {
			t.Logf("got unexpected error: %v", err)
...
func TestTunnelTimeout(t *testing.T) {
	...
	icImage, err := getIngressControllerDeploymentImage(t, kclient, icName, 1*time.Minute)
	if err != nil {
		t.Fatal("failed to determine ingresscontroller deployment's image: ", err)
	}

	// Create a pod with an HTTP application that sends delayed responses.
	// The delay is 40s which is enough to test the default tunnel timeout.
	pod := buildWebSocketServerPod("slow-wssrv", operatorcontroller.DefaultOperandNamespace, icImage, "40s")
	if err := kclient.Create(context.TODO(), pod); err != nil {
		t.Fatalf("failed to create pod %s/%s: %v", pod.Namespace, pod.Name, err)
	}
	defer func() {
