...
func TestAWSLBTypeChange(t *testing.T) {
	...
	// LB Annotation should be unset by default (meaning CLB).
...
func effectuateIngressControllerLBType(t *testing.T, ic *operatorv1.IngressController, expectedLBType operatorv1.AWSLoadBalancerType, expectedOperatorConditions ...operatorv1.OperatorCondition) {
	...
	t.Logf("Recreating service %s to effectuate the LB type", controller.LoadBalancerServiceName(ic))
...
func TestUnsupportedConfigOverride(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := kclient.Get(context.Background(), controller.RouterDeploymentName(ic), &deployment); err != nil {
				t.Fatalf("failed to get ingresscontroller deployment: %v", err)
			}

			if err := waitForDeploymentEnv(t, kclient, &deployment, "router", 1*time.Minute, tt.expectedEnv); err != nil {
...
func waitForDeploymentEnvVar(t *testing.T, cl client.Client, deployment *appsv1.Deployment, timeout time.Duration, name, value string) error {
	...
	return waitForDeploymentFunc(t, cl, deployment, timeout, func(deployment *appsv1.Deployment) bool {
		for _, container := range deployment.Spec.Template.Spec.Containers {
			if container.Name == "router" {
...
func waitForDeploymentEnv(t *testing.T, cl client.Client, deployment *appsv1.Deployment, containerName string, timeout time.Duration, expectedEnv map[string]string) error {
	t.Helper()
...
func waitForDeploymentFunc(t *testing.T, cl client.Client, deployment *appsv1.Deployment, timeout time.Duration, fn func(deployment *appsv1.Deployment) bool) error {
	t.Helper()
