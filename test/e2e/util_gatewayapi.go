	...
	if err := kclient.Create(context.TODO(), httpRoute); err != nil {
		if kerrors.IsAlreadyExists(err) {
			...
			if err = kclient.Get(context.TODO(), name, httpRoute); err == nil {
				...
				return nil, errors.New("failed to access existing http route: " + err.Error())
			}
		} else {
	...
	if err := kclient.Create(context.TODO(), gateway); err != nil {
		if kerrors.IsAlreadyExists(err) {
			...
			if err = kclient.Get(context.TODO(), name, gateway); err != nil {
				return nil, errors.New("failed to get the existing gateway: " + err.Error())
			}
		} else {
...
func assertHttpRouteSuccessful(t *testing.T, namespace, name, openshiftIngressNamespace, hostname string, gateway *gwapi.Gateway) (error, *gwapi.HTTPRoute) {
	...
	if gateway == nil {
		return errors.New("unable to validate httpRoute, no gateway available"), nil
	}
...
func assertHttpRouteConnection(t *testing.T, hostname string, gateway *gwapi.Gateway) error {
	...
	// Wait and check that the dns name resolves first. Takes a long time.
	if err := wait.PollImmediate(20*time.Second, 5*time.Minute, func() (bool, error) {
		_, err := net.LookupHost(hostname)
		if err != nil {
			t.Logf("waiting for HTTP route name %s to resolve...", hostname)
			return false, nil
		}
		return true, nil
	}); err != nil {
...
func assertDNSRecord(t *testing.T, recordName types.NamespacedName) error {
	...
	err := wait.PollImmediate(1*time.Second, 1*time.Minute, func() (bool, error) {
		if err := kclient.Get(context.TODO(), recordName, dnsRecord); err != nil {
