...
func TestGatewayAPI(t *testing.T) {
	getGatewayAPIEnabled(t)
	// Skip if feature is not enabled
	if !gatewayAPIEnabled {
		t.Log("Gateway API not enabled, skipping TestGatewayAPI")
		return
	}

	// Defer the cleanup of the test gateway.
	defer func() {
		testGateway := gwapi.Gateway{ObjectMeta: metav1.ObjectMeta{Name: testGatewayName, Namespace: openshiftIngressNamespace}}
...
func getGatewayAPIEnabled(t *testing.T) {
	t.Helper()
...
func testGatewayAPIIstioInstallation(t *testing.T) {
	...
	if err := assertIstiodProxy(t); err != nil {
		t.Fatal("failed to find expected Istiod proxy")
	}
...
func ensureGatewayObjectCreation(ns *corev1.Namespace) error {
	...
	// The gateway is cleaned up by a defer function in TestGatewayAPIObjects.
...
func ensureGatewayObjectSuccess(t *testing.T, ns *corev1.Namespace) []string {
	...
	if err != nil {
		...
		err = assertHttpRouteConnection(t, default_routename, gateway)
		if err != nil {
			errors = append(errors, error.Error(err))
		}
	}
