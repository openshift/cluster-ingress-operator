...
func TestAWSNLBProtocol(t *testing.T) {
	...
	if err := waitForIngressControllerNLBProtocol(t, name, operatorv1.ProxyProtocol); err != nil {
	...
	if err := updateIngressControllerWithRetryOnConflict(t, name, 5*time.Minute, func(ic *operatorv1.IngressController) {
		ic.Spec.EndpointPublishingStrategy.LoadBalancer.ProviderParameters.AWS.NetworkLoadBalancerParameters.Protocol = operatorv1.TCPProtocol
	}); err != nil {
	...
	if err := waitForIngressControllerNLBProtocol(t, name, operatorv1.TCPProtocol); err != nil {
...
func waitForIngressControllerNLBProtocol(t *testing.T, name types.NamespacedName, expected operatorv1.IngressControllerProtocol) error {
	t.Helper()
