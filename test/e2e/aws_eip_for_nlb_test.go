...
func TestAWSEIPForNLB(t *testing.T) {
	...
	var eipAllocations []configv1.EIPAllocations
	...
	for _, validEIPAllocation := range validEIPAllocations {
		eipAllocations = append(eipAllocations, configv1.EIPAllocations(validEIPAllocation))
	}
	...
	var eipAllocationsForUpdate []configv1.EIPAllocations
	validEIPAllocationsForUpdate, err := CreateAWSEIPs()
	if err != nil {
		t.Fatalf("AWS EIPs failed to get created %s", err)
	}
	for _, validEIPAllocationForUpdate := range validEIPAllocationsForUpdate {
		eipAllocationsForUpdate = append(eipAllocationsForUpdate, configv1.EIPAllocations(validEIPAllocationForUpdate))
	}
	...
	var eipAllocationsForUpdateUsingAutoDelete []configv1.EIPAllocations
	...
	for _, validEIPAllocationForUpdateUsingAutoDelete := range validEIPAllocationsForUpdateUsingAutoDelete {
		eipAllocationsForUpdateUsingAutoDelete = append(eipAllocationsForUpdateUsingAutoDelete, configv1.EIPAllocations(validEIPAllocationForUpdateUsingAutoDelete))
	}
	...
	var eipAllocationsEqualSubnets []configv1.EIPAllocations
	...
	for _, eipAllocationEqualSubnetsString := range eipAllocationsEqualSubnetsString {
		eipAllocationsEqualSubnets = append(eipAllocationsEqualSubnets, configv1.EIPAllocations(eipAllocationEqualSubnetsString))
	}
...
func TestAWSInvalidEIP(t *testing.T) {
	...
	invalidEIPAllocations := make([]configv1.EIPAllocations, len(publicSubnets))
	for i := 0; i < len(publicSubnets); i++ {
		invalidEIPAllocations[i] = configv1.EIPAllocations(fmt.Sprintf("%s-%d", "eip-allocation", rand.Int()))
	}
