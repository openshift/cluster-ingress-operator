...
var _ = g.Describe("[sig-network-edge][OCPFeatureGate:GatewayAPIController][Feature:Router][apigroup:gateway.networking.k8s.io][Serial]", g.Ordered, g.Serial, func() {
	...
	g.BeforeAll(func() {
		...
		enabled, err := oc.AllCapabilitiesEnabled(requiredCapabilities...)
		o.Expect(err).NotTo(o.HaveOccurred())
		if !enabled {
			g.Skip(fmt.Sprintf("Skip this test scenario because not all of the following capabilities are enabled: %v", requiredCapabilities))
		}
		...
		if err != nil && !apierrors.IsAlreadyExists(err) {
			framework.Failf("Failed to create GatewayClass %q", gatewayClassName)
		}
	})
	...
	g.It("Ensure OSSM and OLM related resources are created after creating GatewayClass", func() {
		...
		waitCatalogErr := wait.PollUntilContextTimeout(context.Background(), 1*time.Second, 20*time.Minute, false, func(context context.Context) (bool, error) {
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			if catalog != "READY" {
				framework.Infof("CatalogSource %q is not in ready state, retrying...", expectedSubscriptionSource)
				return false, nil
			}
			framework.Infof("CatalogSource %q is ready!", expectedSubscriptionSource)
			return true, nil
		})
		...
		waitVersionErr := wait.PollUntilContextTimeout(context.Background(), 1*time.Second, 20*time.Minute, false, func(context context.Context) (bool, error) {
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			if csvName == "" {
				framework.Infof("Subscription %q doesn't have installed CSV, retrying...", expectedSubscriptionName)
				return false, nil
			}
			framework.Infof("Subscription %q has installed CSV: %s", expectedSubscriptionName, csvName)
			return true, nil
		})
		...
		waitCSVErr := wait.PollUntilContextTimeout(context.Background(), 1*time.Second, 20*time.Minute, false, func(context context.Context) (bool, error) {
			...
			if csvStatus != "Succeeded" {
				framework.Infof("Cluster Service Version %q is not successful, retrying...", csvName)
				return false, nil
			}
			framework.Infof("Cluster Service Version %q has succeeded!", csvName)
			return true, nil
		})
		...
		waitErr := wait.PollUntilContextTimeout(context.Background(), 1*time.Second, 20*time.Minute, false, func(context context.Context) (bool, error) {
			...
			if err != nil {
				framework.Infof("Failed to get OSSM operator deployment %q, retrying...", deploymentOSSMName)
				return false, nil
			}
			if deployOSSM.Status.ReadyReplicas < 1 {
				framework.Infof("OSSM operator deployment %q is not ready, retrying...", deploymentOSSMName)
				return false, nil
			}
			framework.Infof("OSSM operator deployment %q is ready", deploymentOSSMName)
			return true, nil
		})
	...
	g.It("Ensure custom gatewayclass can be accepted", func() {
		...
		if err != nil {
			framework.Infof("Gateway Class \"custom-gatewayclass\" already exists, or has failed to be created, checking its status")
		}
	...
	g.It("Ensure GIE is enabled after creating an inferencePool CRD", func() {
		...
		waitIstioErr := wait.PollUntilContextTimeout(context.Background(), 1*time.Second, 5*time.Minute, false, func(context context.Context) (bool, error) {
			...
			if err != nil {
				framework.Infof("Failed getting openshift-gateway istio cr: %v", err)
				return false, nil
			}
			if strings.Contains(istioEnv, `"ENABLE_GATEWAY_API_INFERENCE_EXTENSION":"true"`) {
				framework.Infof("GIE has been enabled, and the env variable is present in Istio resource")
				return true, nil
			}
			framework.Infof("GIE env variable is not present, retrying...")
			return false, nil
		})
		...
		waitIstioErr = wait.PollUntilContextTimeout(context.Background(), 1*time.Second, 5*time.Minute, false, func(context context.Context) (bool, error) {
			...
			if strings.Contains(istioEnv, `"ENABLE_GATEWAY_API_INFERENCE_EXTENSION":"true"`) {
				framework.Infof("GIE env variable is still present, trying again...")
				return false, nil
			}
			framework.Infof("GIE env variable has been removed from the Istio resource")
			return true, nil
		})
...
func skipGatewayIfNonCloudPlatform(oc *framework.CLI) {
...
func waitForIstioHealthy(oc *framework.CLI) {
	...
	err := wait.PollUntilContextTimeout(context.Background(), 1*time.Second, 10*time.Minute, false, func(context context.Context) (bool, error) {
		...
		if errIstio != nil {
			framework.Infof("Failed getting openshift-gateway istio cr status: %v", errIstio)
			return false, nil
		}
		if istioStatus != "Healthy" {
			framework.Infof("Istio CR %q is not healthy, retrying...", resource.Name)
			return false, nil
		}
		framework.Infof("Istio CR %q is healthy", resource.Name)
		return true, nil
	})
	o.Expect(err).NotTo(o.HaveOccurred(), "Istio CR %q did not reach healthy state in time", resource.Name)
}

func checkGatewayClass(oc *framework.CLI, name string) error {
	waitErr := wait.PollUntilContextTimeout(context.Background(), 2*time.Second, 10*time.Minute, false, func(context context.Context) (bool, error) {
		...
		if err != nil {
			framework.Infof("Failed to get gatewayclass %s, retrying...", name)
			return false, nil
		}
		...
		framework.Infof("Found gatewayclass %s but it is not accepted, retrying...", name)
		return false, nil
	})
...
func createAndCheckGateway(oc *framework.CLI, gwname, gwclassname, domain string) (*gatewayapiv1.Gateway, error) {
...
func checkGatewayStatus(oc *framework.CLI, gwname, ingressNameSpace string) (*gatewayapiv1.Gateway, error) {
	...
	if err := wait.PollUntilContextTimeout(context.Background(), 2*time.Second, 10*time.Minute, false, func(context context.Context) (bool, error) {
		...
		if err != nil {
			framework.Infof("Failed to get gateway %q: %v, retrying...", gwname, err)
			return false, nil
		}
		...
		for _, condition := range gateway.Status.Conditions {
			if condition.Type == string(gatewayapiv1.GatewayConditionProgrammed) {
				if condition.Status == metav1.ConditionTrue {
					framework.Infof("The gateway controller for gateway %q is programmed", gwname)
					programmedGateway = gateway
					return true, nil
				}
			}
		}
		framework.Infof("Found gateway %q but the controller is still not programmed, retrying...", gwname)
		return false, nil
	}); err != nil {
		return nil, fmt.Errorf("timed out waiting for gateway %q to become programmed: %w", gwname, err)
	}
	framework.Infof("Gateway %q successfully programmed!", gwname)
...
func assertGatewayLoadbalancerReady(oc *framework.CLI, gwName, gwServiceName string) {
	...
	err := wait.PollUntilContextTimeout(context.Background(), 1*time.Second, loadBalancerReadyTimeout, false, func(context context.Context) (bool, error) {
		...
		if err != nil {
			framework.Infof("Failed to get service %q: %v, retrying...", gwServiceName, err)
			return false, nil
		}
		...
		if lbAddress == "" {
			framework.Infof("No load balancer address for service %q, retrying", gwServiceName)
			return false, nil
		}
		framework.Infof("Got load balancer address for service %q: %v", gwServiceName, lbAddress)

		gw, err := oc.AdminGatewayApiClient().GatewayV1().Gateways("openshift-ingress").Get(context, gwName, metav1.GetOptions{})
		if err != nil {
			framework.Infof("Failed to get gateway %q, retrying...", gwName)
			return false, nil
		}
		...
		framework.Infof("Gateway %q does not have service load balancer address, retrying...", gwName)
		return false, nil
	})
...
func assertDNSRecordStatus(oc *framework.CLI, gatewayName string) {
	...
	err := wait.PollUntilContextTimeout(context.Background(), 2*time.Second, 10*time.Minute, false, func(context context.Context) (bool, error) {
		...
		if err != nil {
			framework.Infof("Failed to list DNS records for gateway %q: %v, retrying...", gatewayName, err)
			return false, nil
		}
		...
		framework.Infof("DNS record %q is not ready, retrying...", gatewayDNSRecord.Name)
		return false, nil
	})
...
func createHttpRoute(oc *framework.CLI, gwName, routeName, hostname, backendRefname string) (*gatewayapiv1.HTTPRoute, error) {
	...
	if errGwStatus != nil || gateway == nil {
		framework.Failf("Unable to create httpRoute, no gateway available during route assertion %v", errGwStatus)
	}
	...
	waitErr := wait.PollUntilContextTimeout(context.Background(), 1*time.Second, 4*time.Minute, false, func(context context.Context) (bool, error) {
		...
		if err != nil {
			framework.WarnContextf("error getting gateway, retrying...; %s", err)
			return false, nil
		}
		if len(checkHttpRoute.Status.Parents) < 1 {
			framework.WarnContextf("gateway does not have parents set yet, retrying...; %s", err)
			return false, nil
		}
		...
		framework.Infof("HTTPRoute %s is not ready, retrying...", checkHttpRoute.Name)
		return false, nil
	})
	if waitErr != nil {
		framework.Failf("HTTPRoute never ready")
	}
...
func assertHttpRouteSuccessful(oc *framework.CLI, gwName, name string) (*gatewayapiv1.HTTPRoute, error) {
	namespace := oc.GetNamespace()
	checkHttpRoute := &gatewayapiv1.HTTPRoute{}
	ingressNameSpace := "openshift-ingress"
	gateway, errGwStatus := oc.AdminGatewayApiClient().GatewayV1().Gateways(ingressNameSpace).Get(context.TODO(), gwName, metav1.GetOptions{})
	if errGwStatus != nil || gateway == nil {
		framework.Failf("Unable to assert httproute, no gateway available, error %v", errGwStatus)
	}
	...
	err := wait.PollUntilContextTimeout(context.Background(), 2*time.Second, 4*time.Minute, false, func(context context.Context) (bool, error) {
		...
		if numParents == 0 {
			framework.Infof("HTTPRoute %s/%s has no parent conditions, retrying...", namespace, name)
			return false, nil
		}
		framework.Infof("Found httproute %s/%s with %d parent/s", namespace, name, numParents)
		return true, nil
	})
	if err != nil {
		return nil, err
	}
	framework.Infof("HTTPRoute %s/%s successful", namespace, name)
...
func assertHttpRouteConnection(hostname string) {
	...
	err := wait.PollUntilContextTimeout(context.Background(), 20*time.Second, dnsResolutionTimeout, false, func(context context.Context) (bool, error) {
		...
		if err != nil {
			framework.Infof("[%v] Failed to resolve HTTP route's hostname %q: %v, retrying...", time.Now(), hostname, err)
			return false, nil
		}
		return true, nil
	})
	...
	err = wait.PollUntilContextTimeout(context.Background(), 5*time.Second, 5*time.Minute, false, func(context context.Context) (bool, error) {
		...
		if err != nil {
			framework.Infof("HTTP GET request to %q failed: %v, retrying...", hostname, err)
			return false, nil
		}
		if statusCode != http.StatusOK {
			framework.Infof("Unexpected status code for HTTP GET request to %q: %v, retrying...", hostname, statusCode)
...
func getDefaultIngressClusterDomainName(oc *framework.CLI, timeout time.Duration) (string, error) {
	var domain string

	if err := wait.PollUntilContextTimeout(context.Background(), time.Second, timeout, false, func(ctx context.Context) (bool, error) {
		ingress, err := oc.AdminConfigClient().ConfigV1().Ingresses().Get(ctx, "cluster", metav1.GetOptions{})
		if err != nil {
			framework.Infof("Get ingresses.config/cluster failed: %v, retrying...", err)
			return false, nil
		}
