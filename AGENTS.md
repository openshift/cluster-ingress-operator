...
- Avoid gratuitous changes. Defer unrelated cleanups and fixes to separate
  commits or PRs, and put necessary preparatory refactoring in separate
  commits so the key-change diff remains minimal and easy to review, test, and,
  when necessary, backport.
- Test coverage is expected for new features and bug fixes.
- When generated changes are made with commands such as `make update`, `go get`,
  `go mod tidy`, or `go mod vendor`, list the specific commands in the commit
  message as a four-space-indented code block.
